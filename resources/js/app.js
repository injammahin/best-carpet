...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addToQuote(product, extra = {}) {
        ...
        const newKey = productKey(newItem);

        let items = getQuoteItems();

        const existingIndex = items.findIndex((item) => {
            return (
                productKey(item) === newKey &&
                String(item.colour || '') === String(newItem.colour || '') &&
                String(item.type || item.size_label || '') === String(newItem.type || newItem.size_label || '')
            );
        });
    ...
    function removeQuoteItem(key) {
        const items = getQuoteItems().filter((item) => productKey(item) !== String(key));
        saveQuoteItems(items);
        renderQuoteDrawer();
    }

    function updateQuoteQty(key, delta) {
        const items = getQuoteItems()
            .map((item) => {
                if (productKey(item) === String(key)) {
                    return {
                        ...item,
        ...
        items.forEach((item) => {
            ...
            row.innerHTML = `
                ...
                <div>
                    ...
                    <p class="mt-2 text-sm font-semibold text-mega-black">${money(item.price)}${item.sqm ? ` · ${escapeHtml(item.sqm)}m²` : '/m²'}</p>

                    <div class="mt-3 flex w-fit items-center rounded-[7px] border border-mega-line">
                        <button type="button" class="grid h-8 w-8 place-items-center" data-qty-minus="${escapeHtml(key)}">−</button>
                        <span class="grid h-8 w-9 place-items-center text-sm font-extrabold">${Number(item.qty || 1)}</span>
                        <button type="button" class="grid h-8 w-8 place-items-center" data-qty-plus="${escapeHtml(key)}">+</button>
                    </div>
                </div>

                <button type="button" class="h-8 w-8 text-mega-muted hover:text-mega-orange" data-remove-quote="${escapeHtml(key)}">
                    ×
                </button>
            `;
    ...
    function renderWishlistDrawer() {
        ...
        list.querySelectorAll('[data-wishlist-add-quote]').forEach((button) => {
            button.addEventListener('click', () => {
                const item = getWishlistItems().find((product) => productKey(product) === String(button.dataset.wishlistAddQuote));
                ...
                colourIndexButtons.forEach((button) => {
                    button.addEventListener('click', () => {
                        ...
                        if (selectedColourNode) {
                            selectedColourNode.textContent = selectedColour?.name || 'Select a colour first.';
                        }
    ...
    document.querySelectorAll('[data-add-quote]').forEach((button) => {
        if (button.dataset.jsBound === '1') return;
